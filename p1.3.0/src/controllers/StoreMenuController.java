...
public class StoreMenuController {
    ...
    public Result listStoreProducts() {
        if (App.getLoggedStore().products.size() == 0) {
            return new Result(false, "No products available in the store.\n");
        }
        ArrayList<Product> products = new ArrayList<Product>(App.getLoggedStore().products);
    ...
    public Page formPage(ArrayList<Product> products){
        ...
        for(int i = 0 ; i < products.size(); i++){
            ...
            page.information += "\n";
            page.information +=  "Stock: " + (products.get(i).getOnSaleQuantity() + products.get(i).getRegularQuantity());
            page.information += "\nSold: " + products.get(i).getNumberOfSoldProducts();
            page.information += "\n------------------------------------------------\n";
        }
        return page;
    }
